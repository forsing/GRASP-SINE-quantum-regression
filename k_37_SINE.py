...
# SINE (Scale Invariant Network Embedding)
 
# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: SINE – scale-invariant (normalizovani Laplacian / PPMI)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
SINE (Scale Invariant Network Embedding): simetrično normalizovani Laplacian
L_sym = I − D^{-1/2} A D^{-1/2}; sopstveni vektori daju skala-invarijantno ugrađivanje.
"""
...
def compute_sine_embeddings(M, k=EMBED_DIM):
    A = M[1:40, 1:40].astype(float)
    d = A.sum(axis=1) + 1e-12
    d_inv_sqrt = 1.0 / np.sqrt(d)
    D_inv_sqrt = np.diag(d_inv_sqrt)
    L_sym = np.eye(39) - D_inv_sqrt @ A @ D_inv_sqrt
    vals, vecs = np.linalg.eigh(L_sym)
    idx = np.argsort(vals)
    n_comp = min(k, 39)
    emb = vecs[:, idx[:n_comp]].astype(float)
    if emb.shape[1] < k:
...
def main():
    ...
    emb = compute_sine_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (SINE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (SINE + kvantna regresija, deterministički, strukturno):
    (6, 8, 23, 26, 32, 35, 39)

    Score: 0.5000210064675598
    """
...
Simetrično normalizovani Laplacian:
L_sym = I − D^{-1/2} A D^{-1/2} 
(A = susedstvo 1..39, D = dijagonala stepena + 1e-12) 
(39×39).

Spektralno ugrađivanje: 
prve k sopstvenih vektora od L_sym 
(uz redosled po rastućim sopstvenim vrednostima) (39×k).

Normalizacija u [0, π].

Ovo ugrađivanje je skala-invarijantno 
jer zavisi samo od normalizovane strukture grafa, 
a ne od apsolutnih težina grana (39×k).
"""
